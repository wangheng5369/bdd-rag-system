...
分片策略: Bug Chunk
- h2 标题: 模块边界（一级分片）
- h3 标题: Bug单边界（二级分片，每个 h3 包含完整的 bug 信息）

分片结果: 5个模块 + 8个bug = 13 个 chunks
"""
...
RAW_DIR = PROJECT_ROOT / "knowledge-base" / "kb3_bug_reports" / "raw"
CHUNKED_DIR = PROJECT_ROOT / "knowledge-base" / "kb3_bug_reports" / "chunked"
INDEXED_DIR = PROJECT_ROOT / "knowledge-base" / "kb3_bug_reports" / "indexed"
...
# 向量数据库配置
COLLECTION_NAME = "cce_bug_kb3"


@dataclass
class ChunkInfo:
    """分片信息"""
    title_path: str      # 标题路径，如 "一、 集群模块 / Bug-20230815-001"
...
def split_by_ast(markdown_text: str) -> List[ChunkInfo]:
    ...
    分片规则:
    - h2 标题: 模块边界 (一级分片)
    - h3 标题: Bug单边界 (二级分片，每个 h3 包含完整 bug 信息)
    ...
    for idx, heading in enumerate(headings):
        ...
        if level == 2:
            ...
            # 提取 bug ID（如 Bug-20230815-001）
            bug_id = title.split(":")[0].strip() if ":" in title else title
